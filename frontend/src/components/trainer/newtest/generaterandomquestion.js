...
class GeneraterandomQuestionO extends Component {
  ...
  componentDidMount() {
    this.props.fetchSubjectWiseQuestion(
      this.props.test.newtestFormData.testSubject
    );
  }
  ...
  handleSubmit = (e) => {
    ...
    this.props.form.validateFields((err, values) => {
      if (!err) {
        console.log(values);
        if (
          values.no <= this.props.test.questionsAvailablebasedonSubject.length
        ) {
          var qus = [];
          var allquestions = [
          ...
          Alert(
            "error",
            "Error!",
            "You don't have enough questions to select."
          );
        }
      }
    });
      ...
      <div>
        <Row>
          <Col span={5} style={{ padding: "20px 0px" }}>
            <div
              className={`random-question-generation ${
                this.props.mode === "random" ? "notblind" : "blind"
              }`}
            >
...
const GeneraterandomQuestion = Form.create({ name: "Basic Form" })(
